...
async function persistDirectHlsToGcs(transcodeOutputDir: string, videoId: string): Promise<string> {
  ...
  return `https://storage.googleapis.com/${bucketName}/${destinationFolder}/master.m3u8`;
}
export async function transcodeVideoToLocalHlsDirect(
  ...
  try {
    ...
    if (fs.existsSync(playlistPath) && (await fs.promises.stat(playlistPath)).size > 0) {
      ...
      // index.m3u8 is the media playlist generated by FFmpeg.
      // master.m3u8 is the variant playlist that points to it; never make
      // index.m3u8 point to itself.
